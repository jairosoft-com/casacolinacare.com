...
2. Read the progress log at `progress.txt` (check Codebase Patterns section
   first)
3. Start from `main` — run `git checkout main && git pull origin main` to ensure
   you have the latest code. Then create a new feature branch for the story:
   `git checkout -b ralph/{story-id}-{slugified-title}` (e.g.,
   `ralph/US-001-project-scaffolding`, `ralph/US-002-header-navigation`)
...
11. Push the feature branch to the remote:
    `git push -u origin ralph/{story-id}-{slugified-title}`
...
13. If `gh` CLI is not available or not authenticated, skip the PR creation, log
...
If ALL stories are complete and passing, verify that all feature branches have
been pushed and PRs have been opened. If any PRs are missing, open them before
declaring complete. Then reply with: <promise>COMPLETE</promise>
