...
MISSION: You are an autonomous coding agent implementing user stories for the
CasaColinaCare.com website. Work through one user story per iteration, following
the workflow defined here.
...
3. Start from `main` — run `git checkout main && git pull origin main`, then
   create a feature branch: `git checkout -b ralph/{story-id}-{slugified-title}`
   (e.g., `ralph/US-001-header-navigation`)
...
12. If `gh` CLI is unavailable, log in `progress.txt` that a PR needs manual
    creation
...
- [prd.json](prd.json) — 12 user stories (US-001 through US-012) with acceptance
  criteria
- [prd-casacolinacare-website.md](prd-casacolinacare-website.md) — Full PRD with
  functional requirements and design details
- [SPEC.md](SPEC.md) — Engineering specification with brand theme, file
  structure, API contracts, and test plan
- [../../AGENTS.md](../../AGENTS.md) — Root repo instructions (commands,
  architecture, code style, boundary conditions)
- [../../CLAUDE.md](../../CLAUDE.md) — AI guidance with architecture patterns
...
- If stories remain with `passes: false`, end normally (another iteration picks
  up the next story)
